define([], () => {
	...
	let map = {
		target: undefined, // document.getElementById("full-map"),
		container: undefined,
		center: undefined,
		...
		autoRefresh: false,
		interval: undefined,
		defaultCircuitID: $routeParams.circuitId,
		view: new ol.View({
			zoom: 5,
			maxZoom: 21,
			minZoom: 2.5,
				...
				map.forEachFeatureAtPixel(event.pixel, function (feature, layer) {
					if (layer === self.deviceLayer) {
						if (!devicesInPixel[feature.get("name")])
							devicesInPixel[feature.get("name")] = {
								id: feature.get("id"),
						...
						if (!circuitsInPixel[feature.get("alias")])
							circuitsInPixel[feature.get("alias")] = {
								id: feature.get("id"),
								color: colorMap[feature.get("type")]
							};
					}
				});
